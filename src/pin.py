...
def _schema_of(data: Any, depth=0) -> dict:
    """Extract the shape (types/keys) of a JSON value."""
...
def apipin(
    ...
    ::
        ...
        apipin("github_user", resp)   # saves schema on first call
                                       # raises/warns on future calls if shape drifts

        # When GitHub removes the 'blog' field someday:
        # apipin: DRIFT in 'github_user': .blog key missing in actual response
    """
    sdir = Path(snapshot_dir or snapshot_dir or _SNAPSHOT_DIR)
